...
<div class="container">
    <div class="row mb-3">
        <div class="col-md-12">
            <a href="{{ route('user.index') }}" class="btn btn-sm btn-secondary text-white">
    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('user.update.action', ['user_id' => $user->id]) }}">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <select id="roles" class="form-control" name="roles[]" multiple>
                                    @foreach ($roles as $role)
                                        <option value="{{ $role->name }}" {{ old('roles') ?? $user->roles->contains($role) ? 'selected' : '' }}>{{ $role->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <div class="border rounded p-3 overflow-y-auto">
                                    @include('role.permission-tree', ['permissions' => $permissions, 'mapped' => $mapped])
                                </div>
                            </div>
                        </div>
