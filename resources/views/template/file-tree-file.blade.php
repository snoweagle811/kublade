<li class="d-flex justify-content-between align-items-start flex-row file-tree-li">
    ...
    <div class="file-tree-li-actions">
        <a href="{{ route('template.file.update', ['template_id' => $template->id, 'file_id' => $structure->id]) }}" class="btn btn-sm btn-warning text-white p-1 lh-1">
            <i class="bi bi-pencil file-tree-action"></i>
        </a>
        <a href="{{ route('template.file.delete.action', ['template_id' => $template->id, 'file_id' => $structure->id]) }}" class="btn btn-sm btn-danger text-white p-1 lh-1">
